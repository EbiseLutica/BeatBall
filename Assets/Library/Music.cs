...
public class Music : MonoBehaviour
{
	...
	void Initialize()
	{
		if (musicSource_ != null && musicSource_.clip != null)
		{
	...
	void UpdateTiming()
	{
		...
		if (sectionSample >= 0)
		{
			just_.Bar = sectionSample / samplesPerBar_ + CurrentSection_.StartBar;
			just_.Beat = (sectionSample % samplesPerBar_) / samplesPerBeat_;
			just_.Unit = ((sectionSample % samplesPerBar_) % samplesPerBeat_) / samplesPerUnit_;
			...
			just_.Bar -= CurrentSection_.StartBar;
			timeSecFromJust_ = (sectionSample - just_.Bar * samplesPerBar_ - just_.Beat * samplesPerBeat_ - just_.Unit * samplesPerUnit_) / (double)samplingRate_;
		...
		if (DebugText != null)
		{
			DebugText.text = "Just = " + Just + ", MusicalTime = " + MusicalTime_;
			if (Sections.Count > 0)
			{
